...
async def _generar_gemini_vision_tryon(foto_usuario_bytes: bytes, garment_bytes: bytes, prenda_nombre: str) -> Optional[str]:
    ...
    prompt_text = (
        ...
        f"INPUT:\n"
        f"- Image 1: Photo of a person (any angle, pose, body type, skin tone, camera quality or lighting)\n"
        f"- Image 2: Garment '{prenda_nombre}' (t-shirt/polo/sweater/jacket) in catalog photo\n\n"
        f"PRE-ANALYSIS (before overlaying):\n"
        f"1. Automatically detect the body pose, camera angle, focal distance, and framing of Image 1.\n"
        f"2. Detect the light source, direction, color, and intensity in Image 1.\n"
        f"3. Detect body build (shoulder width, torso, arms) to scale the garment proportionally, without assuming a single body type.\n\n"
        f"GARMENT FITTING:\n"
        f"1. Wrap the garment '{prenda_nombre}' around the torso following the body's real curvature (not as a flat layer stuck on top).\n"
        f"2. Generate natural fabric folds, wrinkles, and drape at flex points (armpits, shoulders, waist), consistent with the detected pose.\n"
        f"3. Apply the garment's own shadow onto the body (under collar, under arms) and the body's shadow onto the garment.\n"
        f"4. Match the garment's white balance, color temperature, and contrast level to the photo, so it doesn't look inserted.\n"
        f"5. Add image grain/noise and sharpness equivalent to original photo, to match quality between garment and person.\n"
        f"6. Align the collar precisely at the base of the actual neck, adjusting to head tilt and shoulder angle.\n"
        f"7. Realistically crop the garment where hidden by arms, hair, hands, or other objects in front (correct occlusion).\n\n"
        f"PRESERVE UNCHANGED:\n"
        f"- Face, expression, facial features, and skin tone\n"
        f"- Hair, hands, arms, and other areas not covered by the garment\n"
        f"- Background, ambient lighting, and original perspective\n"
        f"- The person's real body proportions\n\n"
        f"WORKS FOR:\n"
        f"- Any gender, age, skin tone, or body type\n"
        f"- Vertical or horizontal photos, indoors or outdoors\n"
        f"- Varied poses (facing forward, turned, seated, arms crossed, etc.)\n\n"
        f"AVOID:\n"
        f"- A 'sticker' effect or garment floating without contact with skin/torso\n"
        f"- Covering the face, chin, glasses, or head\n\n"
